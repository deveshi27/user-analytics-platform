...
analytics-api/  
→ Read-only analytics service built with FastAPI.  
→ Exposes metrics APIs backed by Elasticsearch.
...
→ Receives events from clients and publishes them to Kafka.

ingestion-service/  
  app/  
    main.py            → API entrypoint  
    kafka_producer.py        → Kafka producer logic          →   
    models/event_schema.py  → Event validation schemas
    routers/events.py   -> Event ingestion endpoint
  Dockerfile           → Container definition  
  requirements.txt    → Python dependencies  
...
spark-streaming/  
→ Real-time processing and aggregation layer using Spark Structured Streaming.
...
→ Single source of truth for local deployment.
...
airflow/
    dags/  → Workflow orchestration using Airflow.
